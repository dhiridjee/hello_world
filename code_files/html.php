 ...
 * @param $show_id(boolean, default = false) - turns the select text into id + ' -- ' + name
 * @return void
 */
function htmlDrawOptionForReference($col, $selection, $obj=null, $required=false, $critFld=null, $critVal=null, $limitToActiveProjects=true, $show_id = false, &$mapObj = NULL) {   
  ...
  if ($col=='idResource' and $critFld=='idProject') {
    ...
    } elseif ($col == 'idTicketType' && $obj && get_class($obj) == 'Ticket' && $critFld == 'idProject') { //list generic types, as well as dedicated ones, based on idProjectType
        if(!empty($critVal) && $critVal != '*'){
            $prj = new Project($critVal);
            $table = SqlList::getList($listType,$column,$selection, false, " (`idProjectType` IS NULL".($prj->idProjectType ? " OR `idProjectType` = '{$prj->idProjectType}')" : ''));
        }
        else{
            $table = SqlList::getList($listType,$column,$selection, false, " (`idProjectType` IS NULL)");
        }
    } elseif((substr($col, 0, 9) == 'idContext') && !is_null($obj)){
        $class = strtolower(get_class($obj));
        if($class == 'ticketsimple'){
            $class = 'ticket';
        }
        $table=SqlList::getListWithCrit($listType,array($critFld => $critVal, $class => 1),$column,$selection);
    }
    else{
        $critArray=array($critFld=>$critVal);
        $table=SqlList::getListWithCrit($listType,$critArray,$column,$selection);
    }
  } elseif($col == 'idDocumentDirectory') {
        $table = SqlList::getList($listType,$column,$selection, (! $obj)?!$limitToActiveProjects:false, 'idProject='.$_SESSION['project']);
        $wbsList=SqlList::getList($listType,'location',$selection, (! $obj)?!$limitToActiveProjects:false, 'idProject='.$_SESSION['project']);
  } elseif ($col == 'idTicketType'){ // default display only generic ticket types
    $table=SqlList::getList($listType,$column,$selection, (! $obj)?!$limitToActiveProjects:false , '`idProjectType` IS NULL');
  } else {
  ...
  if ($obj) {
    ...
    if ( $class=='Project' and $col=="idProject" and $obj->id!=null) {
      ...
      // if selected value not in restrictArray due to access rights, add it
      if(!array_key_exists($obj->idProject, $restrictArray)){
          $parent = new Project($obj->idProject);
          $restrictArray[$parent->id] = $parent->name;
          unset($parent);
      }
      ...
      if ($obj->id==null) {
        // creation mode
        if ($controlRights["create"]=="PRO" || $controlRights["update"]=="OWN") {
          $restrictArray=$user->getVisibleProjects();
        }
      } else {
        // read or update mode
        if (securityGetAccessRightYesNo($menuClass, 'update', $obj)=="YES") {
          // update
          if ($controlRights["update"]=="PRO" || $controlRights["update"]=="OWN") {
            $restrictArray=$user->getVisibleProjects();
          }            
        }
      }
    } else if ($col=='idStatus' && $class != 'StatusMail') {        
      ...
      if (property_exists($obj,$idType) ) {
        ...
        if ($obj->$idType and $obj->idStatus) {
          ...
          if (property_exists($type,'idWorkflow') ) {
            ...
            $crit=array('idWorkflow'=>$type->idWorkflow, 'allowed'=>1, 'idProfile'=>$profile);
            $acWsList = $ws->getSqlElementsFromCriteria($crit);
            foreach ($acWsList as $ws) {
                $table[$ws->idStatusFrom]=$all_statuses[$ws->idStatusFrom];
            }
            ...
            reset($table);
            next($table); // pick second choice - first is current status
            $fisrtKey = key($table);
            $firstName = current($table);
            ...
            $type = new $typeClass;
            $typeObj = array_shift($type->getSqlElementsFromCriteria(array('idle' => '0')));
            
            
            // get statuses ordered by sortOrder
            $st = new Status();
            $statuses = $st->getSqlElementsFromCriteria(array('idle' => '0'), false, null, "sortOrder");
            $statuses = array_map(function($element){
                return $element->id;
            }, $statuses);
            
            // get first object type's corresponding statuses, ordered by their status table sortOrder column
            $ws=new WorkflowStatus();
            $crit=array('idWorkflow'=>$typeObj->idWorkflow, 'allowed'=>1, 'idProfile'=>$profile, 'idStatusFrom' => $statuses);
            $wsObj = array_shift($ws->getSqlElementsFromCriteria($crit, false, null, "LOCATE (`idStatusFrom`, '".  implode(",", $statuses)."')"));
            
            // select first available status and set it as an option
            $newst = new Status($wsObj->idStatusFrom);
            
           $table=array($newst->id=>$newst->name);
        }
      }
      if ($selection) {
    ...
    // prevent default projeqtor behavior - allow all data from SqlList
...
//    }
  ...
  foreach($table as $key => $val) {
    if (! array_key_exists($key, $excludeArray) and ( count($restrictArray)==0 or array_key_exists($key, $restrictArray) ) ) {
      ...
      if($show_id) {
	echo '><span >'. htmlEncode('#'.$key.' - '.$val) . '</span></OPTION>';
        if(!is_null($mapObj)){
            $mapObj[$key] = '#'.$key.' - '.$val;
        }
      } else {
	echo '><span >'. htmlEncode($val) . '</span></OPTION>'; // AICI
        if(!is_null($mapObj)){
            $mapObj[$key] = $val;
        }
      }
...
// Begin SII Ticket 504 : Base propre
function htmlDisplayInfosSII() {
  global $copyrightIpmp, $versionIpmp, $websiteIpmp;
  echo "<a class='statusBar' href='$websiteIpmp' >$copyrightIpmp $versionIpmp&nbsp;</a>";
}
// Fin SII Ticket 504 : Base propre
