...
abstract class SqlElement {
  ...
  private static $_relationShip=array(
    ...
    "DeliveryDate"    =>  array("DeliveryDate"=>"cascade"),
    ...
    "SeverityRisk" =>           array("Risk"=>"control"),
        ...
        foreach ( $relationShip[get_class($this)] as $object=>$mode) {
          if (($mode=='cascade' or $mode=='confirm') and property_exists($object,'idle')) {
            ...
            foreach ($list as $o) {         
              ...
              if(get_class($o) == 'DocumentVersion' && get_class($this) == 'Document'){
                $resO=$o->save(true);
              }
              else{
                $resO=$o->save();
  ...
  private function copySqlElement() {
    ...
    //ticket 3760
    if (property_exists($newObj,"lastVersionDate")) {
      $newObj->lastVersionDate=null;
    }
     if (property_exists($newObj,"expectedDeliveryDate")) {
      $newObj->expectedDeliveryDate=null;
    }
    if (property_exists($newObj,"deliveryDate")) {
      $newObj->deliveryDate=null;
    }
    if (property_exists($newObj,"internalValidationScheduled")) {
      $newObj->internalValidationScheduled=null;
    }
    if (property_exists($newObj,"externalValidationScheduled")) {
      $newObj->externalValidationScheduled=null;
    }
  // endticket 3760
...
//    begin sii
    if (get_class($this)=='Document') {
      scriptLog('pepito');
      $newObj->idDocumentVersion=NULL;
      $newObj->idDocumentVersionRef=NULL;
      $newObj->version=NULL;
...
//    end sii
  ...
  private function copySqlElementTo($newClass, $newType, $newName, $setOrigin, $withNotes, $withAttachments,$withLinks,$withAssignments=false) {
    ...
    foreach($this as $col_name => $col_value) {
      ...
      if (ucfirst($col_name) == $col_name) {
        ...
        and $col_name!="handled" and $col_name!="handledDate" and $col_name!="handledDateTime"
                 ...
                 and $col_name!="idStatus" and $col_name!="reference" and $col_name != 'reopened'){ //topId ?
    ...
    if (property_exists($newObj,'description') and ! $newObj->description ) {
      ...
      if (property_exists($newObj, $idType)) {
        ...
        /*         * * BEGIN SII Ticket_299 ** */
        if (property_exists($newObj, 'endGarantie') and !$newObj->endGarantie) {
            $idType = 'id' . $newClass . 'Type';
            if (property_exists($newObj, $idType)) {
                $type = $newClass . 'Type';
                $objType = new $type($newObj->$idType);
                if (property_exists($objType, 'mandatoryGuarantee') and $objType->mandatoryGuarantee) {
                    $newObj->endGarantie = $newObj->name;
                }
            }
        }
        /*         * * END SII ** */
      ...
      foreach ($critArray as $colCrit => $valCrit) {
        ...
        if ($valCrit==null or $valCrit==' ') {
          ...
          if ($this->getDataType($colCrit)=='int' and is_numeric($valCrit)) {
                ...
                elseif(is_array($valCrit)){
                    $whereClause .= $this->getDatabaseTableName() . '.' . $this->getDatabaseColumnName($colCrit) . ' IN('.implode(",", $valCrit).")";
          } else {
    ...
    if (Sql::$lastQueryNbRows > 0) {
      ...
      while ($line) {
        ...
        foreach ($obj as $col_name => $col_value) {
                    ...
                    if ($className == "PlannedWork" && $col_name == "work") {
                        $line['work'] = number_format($line['work'], 2);
                    }
            ...
            if (is_array($this->{$col_name}) and ucfirst($colName) == $colName ) {
              if (substr($colName,0,4)=="Link") {
                ...
                $this->{$col_name}=$dv->getSqlElementsFromCriteria($crit,false);
                            //BEGIN_SII
                            } else if ($colName == "DeliveryDate") {
                                //var_dump($this);
                                //echo "<h1>TOTO</h1>";
                                $dd = new DeliveryDate();
                                //$crit=array('id'=>$this->_DeliveryDate);
                                //echo $crit;
                                //$this->{$col_name}=$dd->getSqlElementsFromCriteria($crit,false);
                             //END_SII
              ...
              elseif($colName=="Team"){
                $this->{$col_name}= TeamResource::getTeamsByResource($this->id);
              }
			  elseif( $colName == "TicketWork" && (get_class($this) == "Ticket" || get_class($this) == "TicketSimple")) { // custom implementation for Ticket Refactoring - multiple resources
                $this->{$col_name}= TicketWork::getAssignments($this->id);
              } else {
  ...
  public function getFieldAttributes($fieldName) {
    ...
    //*** Ticket_147 ***//
    public function hideFieldAttribute($fieldName) {
        $fieldsAttributes = $this->getStaticFieldsAttributes();
        if (array_key_exists($fieldName, $fieldsAttributes)) {
            if ($fieldsAttributes[$fieldName] != "hidden") {
                $fieldsAttributes[$fieldName] = "hidden";
            }
    ...
    //*** Ticket_147 ***//
    public function nobrFieldAttribute($fieldName) {
        $fieldsAttributes = $this->getStaticFieldsAttributes();
        if (array_key_exists($fieldName, $fieldsAttributes)) {
            if ($fieldsAttributes[$fieldName] == "hidden") {
                $fieldsAttributes[$fieldName] = "nobr";
            }
            return $fieldsAttributes;
        }
    }
...
//hay q escribir los comentarios 

  public function drawObjectList($crit){
    $result='';
  $result .="<table><tr><td class='label' valign='top'><label>" . i18n($this->getDatabaseTableName()) . "&nbsp;:&nbsp;</label>";
      $result .="</td><td>";
  $result.="<table>";
    $objList=$this->getSqlElementsFromCriteria($crit);
    foreach ($objList as $obj) {
      $result.= '<tr>';
      $result.= '<td valign="top" width="20px"><img src="css/images/iconList16.png" height="16px" /></td>';
      $result.= '<td>';   
      $result.=htmlDrawLink($obj);
      $result.= '</td></tr>';
    }
    $result .="</table>";
   ...
   * @return type
   */
  protected function getStaticFieldsAttributes($force = FALSE) {
      $class = get_class($this);
      if(!$force && isset($_SESSION['staticFieldsAttributes'][$class])){
          return $_SESSION['staticFieldsAttributes'][$class];
      }
      
      $fieldsAttributes = self::$_fieldsAttributes;
      if(in_array($class, Parameter::getGlobalParameter('customizableObjects'))){
        $fieldsAttributes += static::$_fieldsAttributes;
      }
      
      if(!$force && in_array($class, Parameter::getGlobalParameter('customizableObjects'))){
        self::filterStaticFieldsAttributes($fieldsAttributes);
        $_SESSION['staticFieldsAttributes'][$class] = $fieldsAttributes;
      }
  ...
  /**
   * Filters the fieldsAttributes according to the UICustomization parameters
   * @param string $fieldsAttributes
   */
  
  protected function filterStaticFieldsAttributes(&$fieldsAttributes){
        if(!empty($fieldsAttributes)){
            $param = new Parameter();

            $fA = $param->getSqlElementsFromCriteria(null, false, " `idProject` IS NULL AND `idUser` IS NULL AND `parameterCode` LIKE '".(get_class($this))."_%'");

            foreach($fA as $o){
                $code = explode('_', $o->parameterCode, 2);
                $fieldsAttributes[$code[1]] = 'hidden';
            }
        }
  }
  ...
  public function getValidationScript($colName) {
    ...
    if ($colName=="idStatus") {
      ...
      if (property_exists($this, 'idleDateTime')) {
        ...
        $colScript .= '      dijit.byId("idleDateTimeBis").set("value", "T"+curDate.getHours()+":"+curDate.getMinutes()); ';
        $colScript .= '    }';
      }
      ...
      if (property_exists($this, 'doneDateTime')) {
        ...
        $colScript .= '      dijit.byId("doneDateTimeBis").set("value", "T"+curDate.getHours()+":"+curDate.getMinutes()); ';
        $colScript .= '    }';       
      }
      ...
      if (property_exists($this, 'handledDateTime')) {
        ...
        $colScript .= '      dijit.byId("handledDateTimeBis").set("value", "T"+curDate.getHours()+":"+curDate.getMinutes()); ';
        $colScript .= '    }';
      }
      ...
      and property_exists($this, 'description')) {
        if (! $this->description) {
          $result.='<br/>' . i18n('messageMandatory',array($this->getColCaption('description')));
            ...
            /*             * * BEGIN_SII_Ticket#229 ** */
            if (property_exists($objType, 'mandatoryGuarantee') and $objType->mandatoryGuarantee
                    and property_exists($this, 'endGarantie')) {
                if (!$this->endGarantie) {
                    $result.='<br/>' . i18n('messageMandatory', array($this->getColCaption('endGarantie')));
                }
            }

            /*             * * END_SII ** */

            /*             * * BEGIN_SII_Ticket#471 ** */
            if (property_exists($objType, 'mandatoryPlanningActivity') and $objType->mandatoryPlanningActivity
                    and property_exists($this, 'idActivity')) {
                if (!$this->idActivity || $this->idActivity == " ") {
                    $result.='<br/>' . i18n('messageMandatory', array($this->getColCaption('idActivity')));
                }
            }
            /*             * * END_SII ** */
        ...
        if ($this->done and ! $this->result) {
          $result.='<br/>' . i18n('messageMandatory',array($this->getColCaption('result')));
            ...
            if (property_exists($objType, 'mandatoryEfficiencyOnDone') and $objType->mandatoryEfficiencyOnDone
             ...
             //*** Ticket_2256 ***//
          if (property_exists($objType, 'mandatoryResultOnDone') and $objType->mandatoryResultOnDone 
              and property_exists($this, 'idEfficiency')
              and property_exists($this, 'idle')
             and property_exists($this, 'idActionType') ) {
                  if ($this->idle and ! trim($this->idEfficiency)) {
                     $result.='<br/>' . i18n('messageMandatory',array($this->getColCaption('idEfficiency')));
                    }
           }
        ...
        if ($mode=="control" or $mode=="confirm") {
          ...
          if ($nb>0) {
            ...
            if ($mode=="confirm" and self::isDeleteConfirmed()) {
              ...
              if (get_class($this)=="Workflow") {
              $objects = "<br>There are ".$nb." dependencies linked to the workflow.";
              } else {
              $objects.="<br/>&nbsp;-&nbsp;" . i18n($object) . " (" . $nb . ")";
              }
          }
  ...
  $anyChange=false, $attachmentsArray = null) {
    $objectClass=get_class($this);
    ...
    if(!empty($attachmentsArray) && is_array($attachmentsArray) && !empty($this->_Attachement)){
        $pbase = Parameter::getGlobalParameter('paramWebBase');
        $paramFilenameCharset=Parameter::getGlobalParameter('filenameCharset');
        foreach($this->_Attachement as $obj_att){
            if(in_array($obj_att->id, $attachmentsArray)){
                $path = str_replace('${attachementDirectory}', Parameter::getGlobalParameter('paramAttachementDirectory'), $obj_att->subDirectory);
                $name = $obj_att->fileName;
    ...
    $resultMail=sendMail($dest, $title, $message, $this, null, null, $mailAttachments);
    if ($directStatusMail) {
      if ($resultMail) {
...
//BEGIN_SII ticket 3654 ,  we ad the lik elements and the attachaments element in th mail,
      if (isset($this->_Link) and is_array($this->_Link)) {
          ...
          if (get_class($this) == 'Document') {
                $dv = new DocumentVersion();
                $lstVers = $dv->getSqlElementsFromCriteria(array('idDocument' => $this->id));
      ...
      $msg.=chr(10);      
         ...
         foreach ($list as $link) {    
            $linkObj = null;
            if ($link->ref1Type == get_class($this) and $link->ref1Id == $this->id) {
            ...
            if (get_class($linkObj)=='Action')
            {
                $userName = SqlList::getNameFromId('Resource',$linkObj->idResource);
                $creationDate = $linkObj->actualDueDate ;
            }
          ...
          $msg.= '<br/>';
          $msg.= htmlFormatDateTime($creationDate);
//          if($linkObj->ref1Type == 'Document'){
//              $obj= New $link->ref1Type($link->ref2Id);
//          }
//          else{
//              $obj= New $link->ref1Type($link->ref1Id);        
//          }
          $msg.=$labelEnd.$fieldStart;
          $msg.=htmlEncode(get_class($linkObj));
          $msg.="#".htmlEncode($linkObj->id);
          $msg.=" - (";
		  
		  $objStatus = new status($linkObj->idStatus);
		  $msg.=htmlEncode($objStatus->name);
          $msg.=") - ";
		  
          $msg.=(get_class($linkObj) == 'DocumentVersion') ? htmlEncode($linkObj->fullName, 'print') : htmlEncode($linkObj->name, 'print');
          $msg.=$fieldEnd.$rowEnd;    
      }
      
      
    }
       if (isset($this->_Attachement) and is_array($this->_Attachement)) {
          $atta = new Attachement();
          $attas=$atta->getSqlElementsFromCriteria(array('refType'=>get_class($this),'refid'=>$this->id));
      $msg.=chr(10);
           $msg.=$rowStart.$sectionStart.i18n('sectionAttachements').$sectionEnd.$rowEnd;
         foreach ($attas as $atta) {    
          ...
          $msg.= htmlFormatDateTime($creationDate);
          $msg.=$labelEnd.$fieldStart;
          $msg.=htmlEncode($atta->refType);
           $msg.=" ";
          $msg.=htmlEncode($atta->fileName,'print');
